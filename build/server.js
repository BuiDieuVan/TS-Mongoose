...
var db = mongoose.connect('mongodb://localhost/blog-demo-tutorial');
// express middleware
...
if (process.env.NODE_ENV !== main_1.default.test_env) {
    server = app.listen(main_1.default.port, function () {
        console.log("server running on port " + main_1.default.port);
    });
}
else {
    server = app.listen(main_1.default.test_port, function () {
        console.log("Server running on port " + main_1.default.test_port);
    });
}
